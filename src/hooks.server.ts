...
// Bridge the agent WebSocket handler out of the SvelteKit bundle so the
// production server (server/index.js) can route /agent/ws upgrades to it.
// In dev the Vite plugin in vite.config.ts loads the module directly.
export const init: ServerInit = async () => {
	if (building) return;
	const { handleUpgrade } = await import('$lib/server/agents/ws');
	(globalThis as Record<string, unknown>).__finderellaAgentUpgrade = handleUpgrade;
...
// /api/agent/pair authenticates by claim code; /api/stream/* (Phase 2) by
// playback-session capability id and is short-circuited before session lookup.
const PUBLIC_PREFIXES = ['/login', '/api/auth', '/api/agent/pair'];
